...
\section{Relații de echivalență. Mulțimi factor (11-Oct-1994)}
...
\begin{enumerate}
    ...
    \item[(7)] (cu notațiile de la (4) și (6)): $n \in \mathbb{N}^*$, $f: \mathbb{Z} \rightarrow \mathbb{C}$, $f(k) = \cos \frac{2k\pi}{n} + i \sin \frac{2k\pi}{n}$. Atunci $R_f = R_n$. \textit{(pt. demonstrație v. finalul cursului).}
\end{enumerate}
\end{example}


% \textbf{Exemplu (cu notațiile de la (4) și (6)):} $n \in \mathbb{N}^*$, $f: \mathbb{Z} \rightarrow \mathbb{C}$, $f(k) = \cos \frac{2k\pi}{n} + i \sin \frac{2k\pi}{n}$. Atunci $R_f = R_n$. \textit{(pt. demonstrație v. finalul cursului).}

% repeta definitia de mai sus 
% \begin{definition}
...
\begin{enumerate}
    \item reflexivă, simetrică, tranzitivă se numește \textbf{relație de echivalență}.
    \item reflexivă, antisimetrică, tranzitivă se numește \textbf{relație de ordine}.
    \item reflexivă, tranzitivă se numește \textbf{relație de cvasioardine} (preordine).
\end{enumerate}
\end{definition}
...
\begin{itemize}
    \item \textbf{Relație de echivalență:} Congruența modulo $n$ pe $\mathbb{Z}$, adică $(\mathbb{Z}, \equiv \pmod n)$.
    \item \textbf{Relație de ordine:} Relația de incluziune $\subseteq$ pe ansamblul părților unei mulțimi, $(\mathcal{P}(X), \subseteq)$.
    \item \textbf{Relație de preordine:} Relația de divizibilitate pe $\mathbb{Z}$. Este reflexivă și tranzitivă, dar nu este antisimetrică deoarece $a \mid b$ și $b \mid a \implies a = \pm b$.
\end{itemize}
\end{example}
...
\begin{definition}
Fie $A \neq \emptyset$ o mulțime și $\sim$  o relație de echivalență pe $A$. O submulțime $\emptyset \neq C \subseteq A$ se numește \textbf{clasă de echivalență} dacă:
\begin{enumerate}
...
% \begin{example}\mbox{}\\
% $n = 6$, $C = 6\mathbb{Z} + 1 \subset \mathbb{Z}$. \\
...
\begin{example}\mbox{}\\
Considerăm pe $\mathbb{Z}$ relația de congruență modulo 6, definită prin:
$$ R_6 = \{(x, y) \in \mathbb{Z} \times \mathbb{Z} \mid 6 \mid x - y\} $$
Atunci submulțimea $C = 6\mathbb{Z} + 1 \subset \mathbb{Z}$ este o clasă de echivalență pentru relația $R_6$. Verificăm cele două condiții:
\begin{enumerate}
    \item Dacă $c_1, c_2 \in C$, atunci $c_1 = 6q_1 + 1$ și $c_2 = 6q_2 + 1$. Avem $c_1 - c_2 = 6(q_1 - q_2)$, deci $6 \mid c_1 - c_2 \implies c_1 \sim c_2$.
    \item Dacă $a \in \mathbb{Z}$, $c \in C$ și $a \sim c \implies a - c = 6k$. Știind că $c = 6q + 1$, obținem: \\
    $a - (6q + 1) = 6k \implies a = 6(k + q) + 1 \in C$.
\end{enumerate}
\end{example}
...
\begin{proposition}
$A/R = \{\hat{a}_i \mid i \in I\}$ unde $\{a_i\}_{i \in I}$ este un sistem complet de reprezentanți. \\
$(\forall) a \in A, (\exists) a_i$ a.î. $\hat{a} = \hat{a}_i$.
\end{proposition}

\begin{example}\mbox{}\\
$\mathbb{Z}/R_6 = \{\hat{0}, \hat{1}, \hat{2}, \hat{3}, \hat{4}, \hat{5}\}$. În general, $\mathbb{Z}/R_n = \{\hat{0}, \hat{1}, \dots, \widehat{n-1}\}$.
\end{example}
...
\begin{example}\mbox{}\\
$\mathbb{Z} = 6\mathbb{Z} \cup (6\mathbb{Z}+1) \cup \dots \cup (6\mathbb{Z}+5)$.
\end{example}
...
\begin{example}\mbox{}\\
$p: \mathbb{Z} \rightarrow \mathbb{Z}/R_n = \mathbb{Z}_n, \quad p(x) = \hat{x} = n\mathbb{Z} + x$.
\end{example}
...
Dacă $f: A \rightarrow B$ este o funcție compatibilă cu relația de echivalență $R$, atunci $(\exists) f^* : A/R \rightarrow B$ astfel încât triunghiul următor să fie comutativ ($f = f^* \circ p$).
...
\begin{theorem}
Fie $f: A \rightarrow B$ o aplicație de mulțimi, $R$ o relație de echivalență pe $A$ și $p: A \rightarrow A/R$ proiecția canonică.
\begin{enumerate}
    \item $(\exists)$ o aplicație $f^*: A/R \rightarrow B$ a.î. $f^* \circ p = f \iff R \subseteq R_f$. \\
...
``$\impliedby$'': Presupunem că $(\exists) f^*: A/R \rightarrow B$ a.î. $f = f^* \circ p$. \\
...
Astfel, $R_{f_n} = R_n$. Conform teoremei de factorizare, $(\mathbb{Z}_n, +) \cong U_n$ (grupul rădăcinilor complexe de ordin $n$ ale unității).